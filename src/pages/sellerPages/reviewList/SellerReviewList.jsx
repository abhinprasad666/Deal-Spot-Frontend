...
const SellerReviewList = () => {
    ...
    const { sellerReviews, loading, error } = useSelector((state) => state.sellerReviews);
    ...
    return (
        <div className="p-4 max-w-5xl mx-auto">
            <h1 className="text-2xl font-bold mb-4 text-gray-800 dark:text-white">My Reviews ({filteredReviews.length})</h1>

            {loading ? (
                <p className="text-center text-gray-500 dark:text-gray-400">
                    <ButtonLoader
                        bottomMessage={"Let’s see what your customers are saying!"}
                        fullPage={true}
                ...
                <div className="text-center text-gray-500 dark:text-gray-300 py-16">
                    <p className="text-lg font-semibold">You haven't added any reviews yet.</p>
                    <p className="text-sm">They'll appear here once you leave some feedback.</p>
                </div>
            ) : (
            ...
            {showModal && (
                <DeleteReviewModal orderId={selectedReviewId} onClose={handleModalClose} onSubmit={handleModalSubmit} />
            )}
        </div>
    );
};
